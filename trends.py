...
def tweet_location(tweet):
    ...
    valores_tupla = (tweet['latitude'], tweet['longitude'])
...
def extract_words(text):
    ...
    """
    # Retorna uma lista apenas com palavras simple e minúsculas.
...
def analyze_tweet_sentiment(tweet):
    ...
    lista_palavras = tweet_words(tweet)
    qtd_palavras_sentimentais = 0
    valor_sentimental = 0

    for x in lista_palavras:
        if has_sentiment(get_word_sentiment(x)):
            qtd_palavras_sentimentais += 1
            valor_sentimental += sentiment_value(get_word_sentiment(x))
    if qtd_palavras_sentimentais > 0:
        media = (valor_sentimental/qtd_palavras_sentimentais)
        return make_sentiment(round(media, 5))
    return average
...
def find_centroid(polygon):
    ...
    coordenadas = polygon # MUDAR DEPOIS

    while i < (len(coordenadas) - 1):
        somatorio_area += ((coordenadas[i][0] * coordenadas[i + 1][1]) - (coordenadas[i + 1][0] * coordenadas[i][1]))
        somatorio_coord_x += ((coordenadas[i][0] + coordenadas[i + 1][0]) * ((coordenadas[i][0] * coordenadas[i + 1][1]) - (coordenadas[i + 1][0] * coordenadas[i][1])))
        somatorio_coord_y += ((coordenadas[i][1] + coordenadas[i + 1][1]) * ((coordenadas[i][0] * coordenadas[i + 1][1]) - (coordenadas[i + 1][0] * coordenadas[i][1])))
        i += 1

    area = (somatorio_area/2)

    if area == 0:

        coordenada_x = coordenadas[0][0]
        coordenada_y = coordenadas[0][1]

        return (coordenada_x, coordenada_y, int(area))
    else:

        coordenada_x = (somatorio_coord_x/(area * 6))
        coordenada_y = (somatorio_coord_y/(area * 6))



        return ((coordenada_x), (coordenada_y), abs(area))
...
def find_center(polygons):
    ...
    for x in range (0, len(polygons)):
        centroides.append(find_centroid(polygons[x]))
    ...
    for y in range (0, len(centroides)):

        numerador_lat += (centroides[y][0] * centroides[y][2])
        numerador_long += (centroides[y][1] * centroides[y][2])
        denominador += centroides[y][2]


    latitude_centroide = (numerador_lat/denominador)
    longitude_centroide = (numerador_long/denominador)


    return (latitude_centroide, longitude_centroide)
...
def find_closest_state(tweet, state_centers):
    ...
    coordenadas_tweet = tweet_location(tweet)
    distancia_unitaria = 0
    distancias = []
    valor_distancias_estado = []

    for x in state_centers:
        distancia_unitaria = geo_distance((coordenadas_tweet), (state_centers[x]))
        valor_distancias_estado.append((x, distancia_unitaria))

    for y in range(0, len(valor_distancias_estado)):
        distancias.append(valor_distancias_estado[y][1])

    for z in range(0, len(distancias)):
        if min(distancias) == valor_distancias_estado[z][1]:
            return (valor_distancias_estado[z][0])
...
def group_tweets_by_state(tweets):
    ...
    for x in tweets:
        sigla_estado = find_closest_state(x, us_centers)
        tweets_by_state[sigla_estado].append(x)
...
def most_talkative_state(term):
    ...
    dicionario = group_tweets_by_state(tweets)

    print(dicionario.items())'''
    tweets = load_tweets(make_tweet, term)  # A list of tweets containing term
    talk = group_tweets_by_state(tweets) #devolve os estados com o termo 
...
def average_sentiments(tweets_by_state):
    ...
    for i in tweets_by_state:
        qntTweets=0
        valorSent=0
        for x in tweets_by_state[i]:
            if has_sentiment(analyze_tweet_sentiment(x))!=0:
                valorSent=valorSent+(sentiment_value(analyze_tweet_sentiment(x)))
                qntTweets=qntTweets+1
        if qntTweets>0:
            mediasentimento=valorSent/qntTweets
            averaged_state_sentiments[i]=mediasentimento
